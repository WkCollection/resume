<template>
  ...
  <div class="template-preview bg-white" :style="pageStyle" style="height: auto;">
    <!-- 标题 -->
    <div class="text-center py-5 relative" :style="{ borderBottom: `2px solid ${primaryColor}` }">
      <h1 class="text-[1.43em] font-medium inline-block px-6" :style="{ color: primaryColor, letterSpacing: '0.5em' }">个人简历</h1>
      <p class="text-[0.57em] text-gray-400 mt-1" style="letter-spacing: 0.3em;">PERSONAL RESUME</p>
    </div>

    <!-- 顶部信息区 -->
    <header class="px-10 pt-5 pb-4 flex items-start gap-6">
      <!-- 左侧：信息网格 -->
    ...
    <div class="px-10 pb-6">
      <template v-for="modKey in orderedModules" :key="modKey">
        ...
        <section v-if="modKey === 'education' && isVisible('education') && data.education?.length" class="mb-6">
          <div class="section-bar flex items-center gap-2 px-3 py-1.5 text-white text-[0.86em] font-bold" :style="{ backgroundColor: primaryColor }">
          ...
          <div class="mt-3">
            <div v-for="(edu, i) in data.education" :key="i" class="mb-3">
        ...
        <section v-if="modKey === 'workExperience' && isVisible('workExperience') && data.workExperience?.length" class="mb-6">
          ...
          <div class="mt-3">
            <div v-for="(work, i) in data.workExperience" :key="i" class="mb-3">
        ...
        <section v-if="modKey === 'projects' && isVisible('projects') && data.projects?.length" class="mb-6">
          ...
          <div class="mt-3">
            <div v-for="(proj, i) in data.projects" :key="i" class="mb-3">
              <div class="flex items-baseline">
